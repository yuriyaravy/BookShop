...
public class AnnotationCSVWriter {
	...
	public static void wtiteToCSVFile(List<?> entities)
			throws NoSuchFieldException, IOException, IllegalArgumentException, IllegalAccessException {
				...
				try (BufferedWriter buffereWriter = new BufferedWriter(new FileWriter(file))) {
					...
					for (int i = 0; i < entities.size(); i++) {
						...
						for (Field tempField : fields) {
							...
							if (tempField.getAnnotation(CsvProperty.class)
									.propertyType() == PropertyType.CompositeProperty) {
								sortedFields.put(tempField.getAnnotation(CsvProperty.class).columnNumber(),
										AnnotationCSVParser.parseToString(tempField.get(element),
												tempField.getAnnotation(CsvProperty.class).keyField()));
							} else {
								sortedFields.put(tempField.getAnnotation(CsvProperty.class).columnNumber(),
										String.valueOf(tempField.get(element)));
							}
						}
